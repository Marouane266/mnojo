...
=============================================================
 ...
 Features:
   ...
   - WhatsApp notifications via Twilio (réel) + lien de suivi
...
TWILIO_NUM   = "whatsapp:+14155238886"   # numéro sandbox Twilio WhatsApp
...
def send_whatsapp(phone, message, code=None):
    """
    Envoie un vrai message WhatsApp via Twilio si les credentials sont
    configurés, sinon génère un lien wa.me click-to-chat.
    Les deux modes sont loggés dans WHATSAPP_LOG.
    """
    entry = {
        "phone": phone,
        "message": message,
    ...
    # --- Envoi réel via Twilio (si credentials présents) ---
    if TWILIO_SID and TWILIO_TOKEN:
        try:
            from twilio.rest import Client as TwilioClient
            tw_client = TwilioClient(TWILIO_SID, TWILIO_TOKEN)

            # Normaliser le numéro marocain : 06XXXXXXXX → +2126XXXXXXXX
            ...
            entry["sent_via_twilio"] = True
        except Exception as e:
            # En cas d'erreur Twilio, on continue sans planter l'app
            entry["twilio_error"] = str(e)
...
# -------------------------------------------------------------
def _wa_messages():
    """Messages WhatsApp avec lien de suivi dynamique (SITE_URL)."""
    return {
        "fr": {
            "registered": (
                ...
                + SITE_URL + "/client/track/{code}"
            ),
            "ready": (
                "Bonjour ! ✅ Votre voiture est prête.\n"
                "Vous pouvez venir la récupérer. Merci !"
            ),
            "reminder": (
                "Bonjour ! ⏰ Petit rappel : votre voiture (code *{code}*) "
                "vous attend toujours au lavage.\n"
                "Suivez-la ici : " + SITE_URL + "/client/track/{code}"
            ),
            "tracking": (
                "Votre code de suivi est : *{code}*\n"
                + SITE_URL + "/client/track/{code}"
            ),
        },
        "ar": {
            "registered": (
                "مرحباً ! 🚗 تم تسجيل سيارتكم بنجاح.\n"
                "رمز التتبع : *{code}*\n"
                "تابعوا تقدم الغسيل هنا :\n"
                + SITE_URL + "/client/track/{code}"
            ),
            "ready": (
                "مرحباً ! ✅ سيارتكم جاهزة.\n"
                "يمكنكم القدوم لاستلامها. شكراً !"
            ),
            "reminder": (
                "مرحباً ! ⏰ تذكير : سيارتكم (الرمز *{code}*) "
                "لا تزال في انتظاركم.\n"
                "تابعوها هنا : " + SITE_URL + "/client/track/{code}"
            ),
            "tracking": (
                "رمز التتبع : *{code}*\n"
                + SITE_URL + "/client/track/{code}"
